...
function evaluateExpression(expr, symbols) {
    ...
    expr = expr.replace(/\b(0[xX][0-9a-fA-F]+|\d+)(ULL|LLU|UL|LU|LL|L|U)\b/g, '$1');
    // Strip C-style casts that are useful for integer expressions.
    expr = expr.replace(/\(\s*(?:unsigned\s+)?(?:long\s+long|long|int|short|char|size_t)\s*\)/g, '');
...
function parseEnum(kind, enumName, body, symbols, options) {
    ...
    for (const rawPart of parts) {
        ...
        if (expression !== null) {
            ...
            if (value === null) {
                console.warn(`[WARN] Cannot evaluate ${enumName}::${name} = ${expression}`);
...
// Main parser.
// ------------------------------------------------------------
export function parseEnums(src, options) {
    ...
    //
    const enumRegex = /\benum\s+(?:(?:OPEN_ENUM|CLOSED_ENUM|FLAG_ENUM)\s+)*([A-Za-z_][A-Za-z0-9_]*)\s*\{/g;
    let match;
    while ((match = enumRegex.exec(clean)) !== null) {
        ...
        if (closeBrace < 0) {
            console.warn(`[WARN] Unclosed enum: ${enumName}`);
            continue;
        }
        const body = clean.slice(openBrace + 1, closeBrace);
        const members = parseEnum("", enumName, body, symbols, options);
...
if (esMain(import.meta)) {
    if (!process.argv[2]) {
        console.error('Usage: node parse-enums.js <CvEnums.h>');
        process.exit(1);
    }
    const inputPath = path.resolve(process.argv[2]);
    const outputPath = path.resolve('enum.json');
