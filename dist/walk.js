...
const tableCount = JSON.parse(readFileSync('tableCount.json', 'utf8'));
const iterate = JSON.parse(readFileSync('iterate.json', 'utf8'));
const primitiveMap = new Map(primitive.map(type => [type.name, type]));
...
function readPrimitive(type, name) {
    ...
    if (!info || typeof info.size !== 'number') {
        console.log(`Unknown primitive type '${type}' at field '${name}'.`);
        stopped = true;
        return null;
    }
...
function readContainerCount() {
    ...
    if (info.size !== 4) {
        throw new Error(`Unexpected size_t size: ${info.size}`);
    }
    return {
        offset: fieldOffset,
        size: info.size,
...
function getEnumCount(enumType) {
    ...
    for (const tableName of candidates) {
        if (Object.prototype.hasOwnProperty.call(tableCount, tableName)) {
            ...
            if (count === undefined) {
                throw new Error(`Invalid table count for '${tableName}': ${count}`);
            }
            return count;
        }
    }
    throw new Error(`Cannot resolve DB table for enum '${enumType}'. ` +
...
function readType(type, name) {
    ...
    console.log(`Unsupported type '${typeToString(type)}' at field '${name}'.`);
    stopped = true;
    return null;
}
/*
